...
2. Place each instance with `<SKILL_PATH>/scripts/map/msw_map_builder.cjs` (`const map = MapBuilder.read(...); map.placeModel(...); map.write(...)`) so ids, paths, `componentNames`, origin metadata, and per-instance component overrides stay synchronized.
3. `refresh`.
...
0-bis. **If the user's request mentions any UI element** (popup, HUD, button, toast, panel, dialog window, menu, tab, layout, screen, bar/gauge, slot) **OR involves writing/editing `.ui` files**, **BEFORE proposing any plan, options, or questions to the user**:
   ...
   2. **All `.ui` mutations must go through `msw-ui-system`'s builder (`<SKILL_ROOT>/msw-ui-system/scripts/msw_ui_builder.cjs`)** — no direct raw JSON editing or grep. Read existing `.ui` files via the read-side builder (`UIBuilder.read/find/list_entities`) too.
...
16. **`.map` files are builder-first.** Use `<SKILL_PATH>/scripts/map/msw_map_builder.cjs` (`MapBuilder.read/snapshot/placeModel/write`) for covered inspection and mutation so entity ids, paths, component names, origin metadata, and model instance mirrors stay consistent. If `MapBuilder` explicitly does not cover the required operation, make the smallest direct `.map` edit possible, then `refresh` and verify.
    ...
    When called from inside the skill's `scripts/` folder, the relative form is `require("<SKILL_PATH>/scripts/map/msw_map_builder.cjs")` — **always anchor the path relative to the script that calls `require`, not to the skill root**. **Full API / per-operation patterns / coverage gaps / `false`-return handling / cross-flow: [`references/builder-protocol.md`](references/builder-protocol.md) §1 + §4** (read alongside [`references/entity.md`](references/entity.md) for domain context).
