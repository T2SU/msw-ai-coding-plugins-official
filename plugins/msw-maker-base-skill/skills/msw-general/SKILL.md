...
2. Place each instance with `skills/msw-general/scripts/map/msw_map_builder.cjs` (`MapBuilder.read(...).placeModel(...).write(...)`) so ids, paths, `componentNames`, origin metadata, and per-instance component overrides stay synchronized.
3. `refresh`.
...
| `LEA-3004` in the log / "won't move" / "floating in mid-air" / "stuck in wall" / "bouncing off" / "disappears off the map" / "falls off the foothold edge" | **`references/troubleshooting.md` (in full) first**, then the matching map type's `platform-{type}.md` §7 |
| shader / material / outline / glow / blur / pixelate / rainbow / tint / grayscale / vignette / screen filter / lens distortion / wave / ripple / distortion / dissolve / additive / blend mode / hologram / mask / post-process / `MaterialID` / `MaterialId` / `ChangeMaterial` / `_MaterialService` / `.material` file | **`references/material.md` (in full)** — then drive shader catalog / property names / per-component compatibility via `mlua_Document_Retriever` + `mlua_API_Retriever` MCP lookups (do NOT memorize) |
...
| [entity.md](references/entity.md) (+ [entity/map-builder.md](references/entity/map-builder.md)) | `.map` builder-first placement, modelId vs inline, snapshot workflow, RUID, coordinate rules | `.map` editing / entity placement |
...
15. **`.model` files are builder-only — and the builder requires `references/model.md` first.** Do not open or edit `.model` JSON directly for inspection or mutation. **Before using `skills/msw-general/scripts/model/msw_model_builder.cjs` (`ModelBuilder.read` / `snapshot` / `fromTemplate` / `write`), you MUST `Read` [references/model.md](references/model.md) IN FULL FIRST** (no `offset`/`limit`) — see the "Model Work Preflight — **MUST**" section above. Only then invoke the builder so identifiers, value metadata, properties, children, and event links stay consistent.
