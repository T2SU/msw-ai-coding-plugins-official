...
15. **`.model` files are builder-only.** Do not open or edit `.model` JSON directly for inspection or mutation. Use `skills/msw-general/scripts/model/msw_model_builder.cjs` (`ModelBuilder.read/snapshot/fromTemplate/write`) so identifiers, value metadata, properties, children, and event links stay consistent.
